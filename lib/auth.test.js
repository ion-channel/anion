...
describe('Auth', () => {
    describe('Accessors', () => {
        it('should set a token', () => {
            global.document = {}
            Auth.setToken('foo')
            expect(global.document.cookie).to.equal('bearer-token=foo; path=/')
        })

        it('should clear a token', () => {
            global.document = { cookie: 'bearer-token=foo; path=/'}
            Auth.clearToken()
            expect(global.document.cookie).to.have.string('bearer-token=;')
        })
    })
