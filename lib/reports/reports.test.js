    ...
    test('should get an analysis report', () => {
      ...
      return Reports.getAnalysis({ projectId: 'someproject', teamId: 'bestteam', analysisId: 'sillyanalysis' })
        .then(response => {
          ...
          expect(response.analysis.id).to.equal(expectedResponse.analysis.id)
          expect(response.analysis.team_id).to.equal(expectedResponse.analysis.team_id)
          expect(response.analysis.branch).to.equal(expectedResponse.analysis.branch)

          expect(response.report.project.id).to.equal(expectedResponse.report.project.id)
          expect(response.report.project_ruleset.id).to.equal(expectedResponse.report.project_ruleset.id)
          expect(response.report.statuses.id).to.equal(expectedResponse.report.statuses.id)
          expect(response.report.digests.length).to.equal(expectedResponse.report.digests.length)
          expect(response.report.ruleset_evaluation.analysis_id).to.equal(expectedResponse.report.ruleset_evaluation.analysis_id)
        })
    })
  })
})
