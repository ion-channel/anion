...
describe('Reports', () => {
  ...
  describe('Latest Analysis', () => {
    test('should get the latest analysis report of a project', () => {
      nock('http://localhost:8001')
        .matchHeader('Authorization', 'bearer soopersecret')
        .get(
          '/v1/report/getLatestAnalysis?project_id=someproject&team_id=bestteam',
        )
        .reply(200, { data: expectedAnalysisReportResponse })

      return Reports.getLatestAnalysis({
        projectId: 'someproject',
        teamId: 'bestteam',
      }).then(res => {
        expect(JSON.stringify(res)).to.equal(
          JSON.stringify(expectedAnalysisReportResponse),
        )
      })
    })
  })
