...
export class Reports {
  ...
  static getLatestAnalysis({ projectId, teamId }) {
    const params = new URLSearchParams({
      project_id: projectId,
      team_id: teamId,
    })

    return Get(getLatestAnalysisEndpoint, params, Auth.appendHeaders())
      .then(({ data }) => {
        Structs.AnalysisReport(data.data)
        return data.data
      })
...
const getLatestAnalysisEndpoint = '/v1/report/getLatestAnalysis'
