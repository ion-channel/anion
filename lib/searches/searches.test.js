    ...
    test('should search for results', () => {
      nock('http://localhost:8001')
        ...
        .reply(200, {
          ...
          meta: { total_count: expectedResponse.length },
        })
      ...
      return Searches.search('ruby').then(resp => {
        ...
        expect(results.length).to.equal(expectedResponse.length)
        expect(meta.total_count).to.equal(expectedResponse.length)
        expect(results[0].name).to.equal(expectedResponse[0].name)
      })
    })
  })
})
