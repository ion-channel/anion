  ...
  describe('Search', () => {
    test('should search for results', () => {
      nock('http://localhost:8001')
        .get('/v1/search?q=ruby&offset=0&limit=10')
        .reply(200, {
    ...
    test('should search for results with deafult pagination', () => {
      nock('http://localhost:8001')
        .get('/v1/search?q=ruby&offset=0&limit=10')
        .reply(200, {
          data: expectedResponse,
      ...
      return Searches.search('ruby').then(resp => {
        ...
        expect(results).toEqual(expectedResponse)
        expect(meta).toEqual({
          total_count: expectedResponse.length,
          limit: 10,
          offset: 0,
        })
      })
    ...
    test('should search for results with unique pagination', () => {
      nock('http://localhost:8001')
        ...
        .reply(200, {
          data: [expectedResponse[0], expectedResponse[1]],
          meta: { total_count: 2, limit: 2, offset: 0 },
        })

      return Searches.search('ruby', 0, 2).then(resp => {
        const { results, meta } = resp
        expect(results).toEqual([expectedResponse[0], expectedResponse[1]])
        expect(meta).toEqual({
          total_count: 2,
          limit: 2,
          offset: 0,
