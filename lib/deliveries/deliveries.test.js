...
describe('Deliveries', () => {
  ...
  describe('Get Destinations', () => {
    test('should get a list of destinations for a team', () => {
      ...
      return Deliveries.getDestinations({ teamId: 'someteamid' })
        .then(response => {
          expect(response).to.not.be.a('error')
          expect(response.length).to.equal(expectedDestinationsResponse.length)

          expect(response[0].id).to.equal(expectedDestinationsResponse[0].id)
          expect(response[0].team_id).to.equal(expectedDestinationsResponse[0].team_id)
          expect(response[0].location).to.equal(expectedDestinationsResponse[0].location)
        })
    })
  })
})
